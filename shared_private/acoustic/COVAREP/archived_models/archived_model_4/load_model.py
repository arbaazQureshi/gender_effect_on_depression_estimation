...
def load_male_model(location=None):
	...
	Y_male = CuDNNLSTM(100, name = 'male_specific_lstm_layer')(X)
	Y_shared = CuDNNLSTM(100, name = 'shared_lstm_layer')(X)

	Y = Concatenate(axis = -1)([Y_male, Y_shared])

	Y = Dropout(rate = 0.3)(Y)

	Y = Dense(100, activation = 'relu', name = 'male_specific_dense_layer_1')(Y)
...
def load_female_model(location=None):
	...
	Y_female = CuDNNLSTM(100, name = 'female_specific_lstm_layer')(X)
	Y_shared = CuDNNLSTM(100, name = 'shared_lstm_layer')(X)
	...
	Y = Dense(100, activation = 'relu', name = 'female_specific_dense_layer_1')(Y)
	Y = Dropout(rate = 0.25)(Y)

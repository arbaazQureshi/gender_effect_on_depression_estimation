...
COVAREP_train, formant_train, text_train, action_units_train, eye_gaze_train, facial_landmarks_train, head_pose_train, Y_train = load_training_data()
COVAREP_dev, formant_dev, text_dev, action_units_dev, eye_gaze_dev, facial_landmarks_dev, head_pose_dev, Y_dev = load_development_data()
COVAREP_test, formant_test, text_test, action_units_test, eye_gaze_test, facial_landmarks_test, head_pose_test, Y_test = load_test_data()
...
while(current_epoch_number <= total_epoch_count):
	...
	history = model.fit([COVAREP_train, formant_train, text_train, action_units_train, eye_gaze_train, facial_landmarks_train, head_pose_train], Y_train, batch_size = batch_size, epochs = 1)
	
	loss_dev = model.evaluate([COVAREP_dev, formant_dev, text_dev, action_units_dev, eye_gaze_dev, facial_landmarks_dev, head_pose_dev], Y_dev, batch_size = batch_size)
	loss_test = model.evaluate([COVAREP_test, formant_test, text_test, action_units_test, eye_gaze_test, facial_landmarks_test, head_pose_test], Y_test, batch_size = batch_size)
